...
- `gitcommitzh` 展开：先执行 Git 变更审查，输出结构化中文说明和中文提交信息草案；若同条消息明确包含“提交 / commit”，先展示“最终写入 Git 的提交信息原文”，再按这段原文执行本地提交。若暂存区为空且未指定文件子集，则默认自动执行 `git add .` 纳入当前工作区改动，而不是中止。
