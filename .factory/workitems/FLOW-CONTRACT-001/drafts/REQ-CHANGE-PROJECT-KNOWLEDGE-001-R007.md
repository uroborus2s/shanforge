# REQ-CHANGE-PROJECT-KNOWLEDGE-001-R007 项目知识索引与只读项目站点需求候选
...
R007 完整取代 R001–R006，不与旧候选叠加解释。旧候选的原子发布、可重建和无 AI 快路径要求继续保留；权限合同修正为可验证的发布 profile：本机 Owner 的离线文件由 OS 权限保护，共享或动态受限内容必须重新鉴权后提供，不能宣称可收回已经打开或复制的离线明文。
...
- 主链为 `queued -> running -> projection_ready -> memory_ready -> html_published -> integrated -> done`；若允许维护提交，则从 `html_published -> maintenance_committed -> integrated`。旁路状态包括 `superseded`、`retryable_failed`、`ready_to_integrate`、`needs_attention`、`commit_not_authorized`。R007 机器合同为每个非终态定义互斥且穷尽的事件集合；发布/集成成功事件必须同时满足输入仍当前、lease 有效和 fencing token 匹配，否则分类为 `INPUT_SUPERSEDED`、`LEASE_LOST`、`TRANSIENT_FAILURE` 或 `PERMANENT_FAILURE` 并走唯一后继。
...
39 表的字段级 schema、索引和约束属于下一阶段正式设计；本需求 Gate 使用 R007 PM field map 证明 137 字段全部具有唯一投影 Owner。映射中的 `field_values_json` 是投影扩展列，不代表正文事实仓；正式设计必须把高频筛选、连接和排序字段提升为有类型列，并保持 field map 兼容。目录 3 字段属于非持久化 `PmNavigationDTO/v1`；canonical `ProjectProgressSnapshot/v2` 不新增 SQLite 历史表。

R007 Markdown 与机器合同采用 `RequirementProjectionCanonicalization/v1` 校验：解析每个 REQ 的 ID、标题、优先级、规范正文和四条 AC，解析每个 NFR 的标题、指标与验证方式，按 UTF-8、LF、NFC、trim-trailing-space 规范化后生成 JCS 对象和 SHA-256；机器合同保存同一对象与逐节 Hash。任何 statement、priority、指标、验证或安全/状态引用不一致都使候选无效，不能只校验 ID 和数量。
...
R007 尚未通过同一独立 Reviewer 的复审，也没有精确 Hash 人工确认。复审通过后，候选 Markdown、完整机器合同、PM field map、R014 pin、review 和 Manifest 将冻结为一个精确候选。`uroborus` 确认该 Manifest Hash 前，不得融入正式 PRD/设计、迁移现有资料或修改产品代码；确认只授权本需求进入设计与实施，不自动批准 R014 或 `TASK-IMPLEMENT-002-R001`，也不授权 Git push、PR、部署或远程动作。
