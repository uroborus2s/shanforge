# REQ-CHANGE-PROJECT-KNOWLEDGE-001-R008 项目知识索引与只读项目站点需求候选
...
R008 完整取代 R001–R007，不与旧候选叠加解释。旧候选的原子发布、可重建和无 AI 快路径要求继续保留；权限合同修正为可验证的发布 profile：本机 Owner 的离线文件由 OS 权限保护，共享或动态受限内容必须重新鉴权后提供，不能宣称可收回已经打开或复制的离线明文。
...
- 137 字段逐项 Owner、目标表/DTO、行模型、源 record ID path、命名空间化目标键、父键来源、缺失/碰撞规则、基数、reducer 和历史策略由 `REQ-CHANGE-PROJECT-KNOWLEDGE-001.pm-field-map.R007.json` 约束，SHA-256 为 `1b9f283483d03b63fea749c4cdda1fa918b20ed70e33f1f3ff114eb4765293f0`。所有值均为 `ProjectProgressSnapshot/v2` 精确 `snapshot_path` 的直接读取，上游派生只归 R014 reducer/rule registry，PM 投影不得重新推导。
...
- 主链为 `queued -> running -> projection_ready -> memory_ready -> html_published -> integrated -> done`；若允许维护提交，则从 `html_published -> maintenance_committed -> integrated`。旁路状态包括 `superseded`、`retryable_failed`、`ready_to_integrate`、`needs_attention`、`commit_not_authorized`。R008 机器合同为每个非终态定义互斥且穷尽的事件集合；发布/集成成功事件必须同时满足输入仍当前、lease 有效和 fencing token 匹配，否则分类为 `INPUT_SUPERSEDED`、`LEASE_LOST`、`TRANSIENT_FAILURE` 或 `PERMANENT_FAILURE` 并走唯一后继。
...
R008 尚未通过同一独立 Reviewer 的复审，也没有精确 Hash 人工确认。复审通过后，候选 Markdown、完整机器合同、PM field map、R014 pin、review 和 Manifest 将冻结为一个精确候选。`uroborus` 确认该 Manifest Hash 前，不得融入正式 PRD/设计、迁移现有资料或修改产品代码；确认只授权本需求进入设计与实施，不自动批准 R014 或 `TASK-IMPLEMENT-002-R001`，也不授权 Git push、PR、部署或远程动作。
