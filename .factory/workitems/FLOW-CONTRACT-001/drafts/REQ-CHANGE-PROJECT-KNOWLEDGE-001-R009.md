# REQ-CHANGE-PROJECT-KNOWLEDGE-001-R009 项目知识索引与只读项目站点需求候选
...
R009 完整取代 R001–R008，不与旧候选叠加解释。旧候选的原子发布、可重建和无 AI 快路径要求继续保留；权限合同修正为可验证的发布 profile：本机 Owner 的离线文件由 OS 权限保护，共享或动态受限内容必须重新鉴权后提供，不能宣称可收回已经打开或复制的离线明文。
...
R009 直接采用 R014 已定义的 canonical `ProjectProgressSnapshot/v2`，不再创建同名低版本 DTO。它由 R014 reducer 从权威来源形成，PM 映射只读取其字段，不再次推导；页面 Manifest、输入指纹和当前输出登记在 `pk_render_view`，项目历史仍由 Git 与权威 Ledger 保存。

R009 使用已经正式发布的 R014 作为精确输入：机器合同内部冻结字段仍为 `candidate_unapproved`，但当前发布状态由 `TASK-REQ-002-R014-release-manifest.json` 决定。绑定 `contract_id=REQ-CONTRACT-WF-CTL-010-001`、`revision=R014`、whole-file SHA-256 `836fadc2c214ef2f56b2a21ef2fb705445a58ca7ddb0047f3b638292ba578d33`、field catalog SHA-256 `658f8d805ce423e46b686e7e6da2de22d0d7e874817a153bffefdf8c0d604313`、release manifest SHA-256 `ea84805f62b9c20d17f625e0e4f68efcd510c19897cc1b5c8ebacf70a5bdef4e` 和人工批准 Ledger key `FLOW-CONTRACT-001:TASK-REQ-002:R014:human-formalization-approved:v1`。任一绑定变化都使本候选失效。
...
- 优先级：P0。
- 项目管理页面必须覆盖上述已正式发布并精确绑定的 R014 输入中的 10 个业务模块和 137 个唯一字段：成员与干系人、工作计划、WBS/任务分解、进度计划、风险、沟通、会议与行动项、状态报告、变更、收尾；字段来源为正式事实或确定性推导。
...
- 137 字段逐项 Owner、目标表/DTO、行模型、源 record ID path、命名空间化目标键、父键来源、缺失/碰撞规则、基数、reducer 和历史策略由 `REQ-CHANGE-PROJECT-KNOWLEDGE-001.pm-field-map.R009.json` 约束，SHA-256 为 `17af8c254017bc60eb44e73b8e61322bc57eb577ffa6baa2711f100d48251055`。所有值均为 `ProjectProgressSnapshot/v2` 精确 `snapshot_path` 的直接读取，上游派生只归 R014 reducer/rule registry，PM 投影不得重新推导。
...
- 主链为 `queued -> running -> projection_ready -> memory_ready -> html_published -> integrated -> done`；若允许维护提交，则从 `html_published -> maintenance_committed -> integrated`。旁路状态包括 `superseded`、`retryable_failed`、`ready_to_integrate`、`needs_attention`、`commit_not_authorized`。R009 机器合同为每个非终态定义互斥且穷尽的事件集合；发布/集成成功事件必须同时满足输入仍当前、lease 有效和 fencing token 匹配，否则分类为 `INPUT_SUPERSEDED`、`LEASE_LOST`、`TRANSIENT_FAILURE` 或 `PERMANENT_FAILURE` 并走唯一后继。
...
- AC-4：不得在主任务活跃工作树中并发修改或提交；隔离 worktree 使用单调 fencing token。模型测试必须枚举每个非终态的全部声明事件并证明恰有一条转移、所有路径在有限重试或人工介入后到达 `done|superseded|needs_attention`。
...
39 表的字段级 schema、索引和约束属于下一阶段正式设计；本需求 Gate 使用 R009 PM field map 证明 137 字段全部具有唯一投影 Owner。映射中的 `field_values_json` 是投影扩展列，不代表正文事实仓；正式设计必须把高频筛选、连接和排序字段提升为有类型列，并保持 field map 兼容。目录 3 字段属于非持久化 `PmNavigationDTO/v1`；canonical `ProjectProgressSnapshot/v2` 不新增 SQLite 历史表。

R009 Markdown 与机器合同采用 `RequirementProjectionCanonicalization/v1` 校验：解析每个 REQ 的 ID、标题、优先级、规范正文和四条 AC，解析每个 NFR 的标题、指标与验证方式，按 UTF-8、LF、NFC、trim-trailing-space 规范化。逐节 Hash 的前像是不含 `projection_sha256/status/markdown_section_id` 的单个规范对象。Root 前像固定为 `RequirementProjectionRoot/v1` 的 JSON 值 `{\"requirements\":[<上述 REQ 规范对象，文档顺序>],\"nfrs\":[<上述 NFR 规范对象，文档顺序>]}`；Root 不包含 schema 字段、逐节 Hash 或其他派生字段。该值经 RFC 8785 JCS 后取 SHA-256，机器合同必须记录规范字节长度与 root Hash。任何字段或 Hash 不一致都使候选无效。
...
R009 尚未通过同一独立 Reviewer 的复审，也没有精确 Hash 人工确认。复审通过后，候选 Markdown、完整机器合同、PM field map、R014 pin、review 和 Manifest 将冻结为一个精确候选。`uroborus` 确认该 Manifest Hash 前，不得融入正式 PRD/设计、迁移现有资料或修改产品代码；确认只授权本需求进入设计与实施；R014 已由既有 release manifest 正式发布，但不自动批准 `TASK-IMPLEMENT-002-R001`，也不授权 Git push、PR、部署或远程动作。
