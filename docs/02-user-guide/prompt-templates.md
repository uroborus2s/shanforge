...
|---|---|---|---|
| 空目录新项目 | 先初始化，不直接手写伪骨架 | [3](#3-空目录新项目初始化) | 生成项目规则入口、内部控制面和 `docs/`，再进入需求阶段 |
| 新项目但目录状态不确定 | 先判断目录是否为空 | [4](#4-新项目但你不确定目录是否为空) | AI 先告诉你应该初始化、做接入诊断还是补规则入口 |
| 已有项目接入 | 先建立真实状态基线 | [5](#5-已有项目接入) | 产出当前状态摘要、当前阶段判断、首批 BUG / CR / TASK 建议 |
| 半初始化项目补齐规则入口 | 先补项目规则入口 | [6](#6-半初始化项目补齐规则入口) | 补齐规则入口，再做状态诊断和阶段检查 |
| 已接入平台后继续工作 | 先读取规则和当前状态 | [7](#7-已接入平台后继续工作) | AI 先确认当前阶段和第一步，再推进本轮目标 |
...
3. 然后再启动需求阶段
4. 不要跳到实现阶段
5. 如果目录非空，不要伪初始化，直接告诉我应改走“已有项目接入”还是“补齐规则入口”
```
...
2. 先判断项目是否已经完成平台接入；如果没有，停止本轮升级并告诉我应先走“已有项目接入”
