...
本节是 `HUMAN-RESPONSE-CONTRACT-002` 的候选修订，待正式批准/发布后才会纳入正式版本。
...
Bug 或修复更新会说明问题现象、影响、复现、已知原因、风险、修复状态和回归验证。任务卡会按事实分流：当前未验收任务内、同一根因且仍在原范围内的问题直接整改；已交付功能的回归、跨任务问题或需要独立排期的问题创建单独的 Bug TaskCard；测试预期、夹具、脚本、配置或环境问题则交给对应 owner 处理，不作为产品 Bug 新建任务卡。
