...
- `SF-SP-008`：当前已完成首版 PR 闭环与提交规则收口并进入 `ready_for_review`。范围是让 `gitcommitzh` 与 review / evidence / memory sync / work item ledger 对齐；提交前必须核对 review、evidence、memory sync 和 work item ledger；`gitcommitzh` 只做本地提交，不创建、不推送、不合并 PR。
