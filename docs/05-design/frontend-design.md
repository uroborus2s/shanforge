...
总览以任务看板为第一信息层级，不再把原始任务流水账平铺为长列表。看板固定为“待开始、进行中、测试中、待评审、待确认 / 阻塞、已完成”六列；每列按权威任务更新时间倒序展示，首页最多直接显示最近 10 条，其余内容放入原生“更多（数量）”折叠区。卡片只显示可理解的中文任务标题，不显示机器任务编号、原始英文状态或下一动作；点击卡片进入带返回按钮的独立任务详情页，机器编号仅在详情的身份区展示。

中文标题依次取任务事件显式标题、任务编号的已登记中文名称、任务简报标题和可解释的稳定中文映射。标题为“任务简报”等通用词时，提取器读取简报中的任务名称；同一机器任务出现多条历史投影时只保留更新时间最新、状态信息最完整的一条。父任务已完成且更新时间不早于子任务时，未关闭子任务在展示投影中同步归档，但不反写或伪造权威任务流水账。

任务机器身份与展示标题是两个字段：机器身份只从 `details.task_id`、
`pk_work_item.work_item_id` 或 `entity_id` 取得，用于去重、父子判断、路由和详情中的
“任务编号”；`display_name/task_title` 只用于中文标题。含 `PRD`、`Markdown`、
`SQLite` 等必要技术词且同时具有清晰中文语义的登记标题允许直接展示，不能因存在
拉丁字母退化为“任务标题待补充”，也不能反过来把中文标题写入机器 ID。

任务事件索引兼容早期缺少 `event_uid`、`idempotency_key` 和 `action_key` 的追加记录：根据事件语义摘要和同内容出现序号生成确定性派生 ID，保留诊断并继续投影后续状态。新事件仍必须提供显式稳定 ID。这样独立评审和人工确认等后续事实不会因旧格式被丢弃，也不会把已完成任务错误显示为“等待独立评审”。独立评审仍是实现候选形成后的内部自动步骤；页面只展示权威记录中已发生的结果，不凭页面渲染自动批准历史任务。

项目管理字段使用“已有数据、待补充、暂无数据源、不适用”四态。“暂无数据源”表示当前项目没有为该字段登记权威来源，不表示任务受阻；“待补充”表示字段适用但来源中没有有效值。页面必须在项目管理入口直接解释四态含义，禁止只显示难以理解的“未登记”。
...
需求列表只展示功能需求和非功能需求，并按 PRD 中的中文能力分类组织；验收标准不再
作为零散顶层需求，而是在所属需求详情中按顺序展示。需求详情的设计、任务、代码和
测试关系，以及任务详情的关联需求，都按目标实体类型生成可点击深链。验收标准关系
指向所属需求页内的稳定锚点，不生成不存在的独立详情页面。

任务详情固定把追踪信息拆成“关联需求”“相关设计”“代码、测试与交付”三个面向人的
区域。关联需求只接受从 Task 出发的登记强关系；相关设计只接受 Requirement 视角下
进入的强 `SATISFIES` 关系，并由 `Task → Requirement ← SATISFIES Design`
确定性推导后直接链接设计详情页。直接 `Task → Design`、弱关系、其他关系类型、标题
或全文猜测都不能进入相关设计区。没有关系时使用完整中文
