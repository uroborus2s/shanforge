...
总览以任务看板为第一信息层级，不再把原始任务流水账平铺为长列表。看板固定为“待开始、进行中、测试中、待评审、待确认 / 阻塞、已完成”六列；每列按权威任务更新时间倒序展示，首页最多直接显示最近 10 条，其余内容放入原生“更多（数量）”折叠区。卡片只显示可理解的中文任务标题，不显示机器任务编号、原始英文状态或下一动作；点击卡片进入带返回按钮的独立任务详情页，机器编号仅在详情的身份与来源区展示。
...
业务阅读。该合同使用 `ProjectSiteRenderer/v7`，因此旧页面指纹自动失效；输入不变时
后续快照仍复用最后有效页面。
