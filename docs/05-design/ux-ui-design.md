...
| 正式版本 | `v1.1.1`（`v1.2.0` 候选修订待评审） |
| 变更来源 | `PM-DASHBOARD-004`（`v1.1.1`）；`TASK-DESIGN-001-R019`（`v1.2.0` 候选） |
| 发布事务 | `DESIGN-RELEASE-TX-R019-G001` |
| 负责人 | `HUMAN_UX_LEAD` |
| 修改 / 审核 / 批准 | `uroborus` / `uroborus` / `uroborus` |
| 状态 | `v1.1.1` 已批准并生效；`v1.2.0` 候选修订待评审 |
...
首页按“当前交付、已批准待办、待治理、最近归档、系统工作”组织任务，不再把内部执行状态平铺成六列。阻塞和人工确认优先，其次是执行中、测试/评审和待办；同级按 `P0 → P1 → P2 → 待补充` 排序。

同一稳定任务 ID 只显示一张卡：任务简报解释目的、范围、优先级和需求关系，ledger 当前记录提供真实状态、更新时间、阻塞和下一步。卡片必须直接展示这些业务字段，不能要求用户先打开详情页才能理解任务在做什么。

系统工作不计入产品完成度。需求、任务和验收证据尚未形成完整追踪链时，首页显示“产品完成度暂不可计算”，不得以所有历史任务的完成数量生成百分比。
