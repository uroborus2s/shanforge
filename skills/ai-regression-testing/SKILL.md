...
description: AI 辅助开发中的回归测试策略。包括无数据库依赖的沙盒模式 API 测试、自动化 Bug 检查工作流、Bug 根因验证，以及针对“同一模型既写代码又审代码”导致的 AI 盲点捕捉模式。
---
...
4. 修复点
5. 防回归测试
6. 是否新增兜底/降级；若新增，说明它为什么是契约要求

根因不清楚时，只允许补诊断或复现测试，不允许提交行为修复。
...
用户: "バグチェックして" (或 "/bug-check")
  ...
  │   └─ 通过 → 继续
  │
  ├─ 第 3 步: 根因核对 (确认不是兜底式修复)
  │   └─ 根因不清楚 → 只补诊断或复现测试
  │
  ├─ 第 4 步: AI 代码评审 (带着已知的盲点进行)
  │   └─ 报告发现的问题
  │
  └─ 第 5 步: 针对每个修复，编写回归测试
