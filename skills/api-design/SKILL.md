...
- 正式 API 用例使用稳定 `TEST-API-*` ID，并建立 `需求 -> 任务 -> 测试 -> 证据` 追踪。
- 每个接口案例至少定义：method/path、角色或认证态、参数 / body fixture、预期状态码、响应 schema / 业务码、
  关键副作用；写接口按风险补幂等、事务和重复请求结果。body 与预期优先由项目现有测试脚本断言，不复制到测试报告。
- 每个契约变更至少标明对应的 schema/contract test；中高风险变更还要覆盖鉴权、错误响应、兼容性和目标集成路径。
- 启动真实 HTTP 服务时，evidence 记录启动命令、端口、健康检查和关闭方式。
- 进程内 transport、测试客户端或纯 schema 检查没有独立监听端口时，端口和关闭方式写 `N/A` 并说明进程内入口。
- 确实不适用的测试层级写 `N/A` 和原因，不得写“待补充”；缺少契约或环境信息时输出 `needs: tests`。
- 案例目录和测试报告只写环境别名与证据引用；不得保存完整内部 URL、IP、端口、凭证、令牌、DSN 或原始敏感日志。
