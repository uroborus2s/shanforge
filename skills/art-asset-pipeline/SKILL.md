...
description: 独立美术与游戏资源管线。用于需要先生成独立美术样张或游戏概念图供用户确认，再生成资源清单，并在清单确认后生产不属于 UI 项目流程的独立美术或游戏资源包的任务。
---

# 独立美术方向 + 资源管线

用于把不属于 UI 项目流程的独立美术或游戏资源探索变成可交付的开发资源。核心原则是先确认方向，再确认清单，最后生产资源包；不承接 UI 项目中的素材阶段。

## 何时使用

- 用户需要不属于 UI 项目流程的独立美术方向、游戏概念图、角色 / 场景 / 道具概念图。
- 用户需要不属于 UI 项目流程的独立美术资源包，例如插画、启动图、背景图、纹理和品牌视觉素材。
...
- UI 项目中的高保真、方向确认、资源清单和正式 UI 素材阶段；由 `ui-ux-pro-max` 完整承接。
- 不含 UI 设计且只需单张最终图片；直接使用 `imagegen`。
...
1. 明确目标类型：独立美术资源包、游戏开发资源包，或只做美术方向探索。
2. 收集约束：目标用户、平台、尺寸、主题、禁用元素、品牌色、分辨率、透明背景、动画帧和导出格式。
3. 使用 `imagegen` 生成 2-4 张独立美术样张或游戏概念图，写入 `candidates/`。
...
- 所有最终资源的缩略图预览。
- 文件名、尺寸、用途和透明背景检查底纹。
- 独立美术资源按页面 / 状态分组；游戏资源按角色、地块、道具、特效和 UI 分组。
- 本地打开即可查看，不依赖远端服务。
...
- 独立美术资源必须检查目标平台尺寸、暗色 / 亮色适配和空状态 / 图标可读性。
