...
description: React 前端开发模式，涵盖状态管理、性能优化以及 UI 最佳实践。
---

# 前端开发模式

适用于 React及高性能用户界面的现代前端开发模式。

## 何时激活

- 构建 React 组件（组合模式、Props、渲染）
- 管理状态（useState, useReducer, Zustand, Context）
- 实现数据拉取（SWR, React Query, 服务端组件 (RSC)）
- 优化性能（记忆化 (memoization), 虚拟列表 (virtualization), 代码分割 (code splitting)）
- 处理表单（验证、受控输入、Zod 模式验证）
- 处理客户端路由与导航
- 构建可访问性良好 (a11y) 的响应式 UI 模式
...
export function CreateMarketForm() {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.name.trim()) {
      ...
      newErrors.name = '名称长度不能超过 200 个字符'
    }
...
**谨记**：现代前端模式使得构建可维护且高性能的用户界面成为可能。请选择能够匹配您项目实际复杂度的模式。
