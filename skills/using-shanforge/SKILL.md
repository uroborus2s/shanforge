...
只要输入包含测试/验证基线，面向用户回复必须使用固定行 `测试基线：total <值>；passed <值>；failed <值>；error <值>；blocked <值>；skipped <值>；not_run <值>；cancelled <值>`，八个标签逐项出现，数值为 0 的类别也必须输出，不得省略；可得事实缺失时对应值写无法计算，不得猜。并逐项保留每个 failed/error 的 TEST-ID、关联功能、现象和当前归因；不得为了摘要或 humanizer 润色省略。最后一次完整测试基线与修复后的定向重跑必须分开报告；定向用例转为 passed 不能改写或重算最后一次完整基线，直到完整集合重跑。每个 failed/error 的 owner 只能来自该用例自身事实；未提供时写未分配/待确认，不得从兄弟用例或模块推断/继承。
