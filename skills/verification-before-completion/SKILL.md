...
- 项目或阶段声明必须逐项核对完整批准需求与变更的追踪矩阵；缺完整基线明确未知，不估算。
- 必须运行完整命令。
- 必须读取完整输出。
- 必须检查 exit code。
- 必须统计失败数量、跳过项和未运行项。
...
3. 运行完整命令；只有完整命令因外部服务、受限硬件、不可获得凭据或明确不在本次变更影响范围而不能运行时，才可使用局部替代验证。替代项必须直接覆盖同一声明的受影响路径，记录未运行完整命令、不可运行原因、替代命令及其 exit code、未覆盖范围和残余风险；不能用 lint、关键词检查或旧结果替代测试、build、发布或关闭验证。
4. 读取完整输出。
5. 检查 exit code。
