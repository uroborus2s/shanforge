...
| 任务身份 | 是否有 `## Work Breakdown` 四列表 `id | parent_id | title | status`，每张 TaskCard 是否映射唯一 `wbs_id`，且 TaskCard、ledger、session 都有一致的 `task_card_id`、`wbs_id`、`current_gate`、`next_required_action` |
