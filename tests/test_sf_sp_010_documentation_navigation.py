...
def test_superpowers_plan_next_steps_are_current() -> None:
    ...
    assert "当前只剩 `SF-SP-010` 文档、导航、memory 同步收口" in plan
    assert "目标文档必须一并纳入同一可审阅提交范围" in plan
...
def test_sf_sp_010_removes_stale_sf_sp_009_progress_language() -> None:
    plan = read(
        "docs/04-project-development/05-development-process/superpowers-workflow-integration-plan.md"
    )
    ...
    for content in (plan, tasks, current_state):
        assert "`SF-SP-009` 已提交为 `9296f58`" in content
        assert "`SF-SP-010` 已进入文档、导航、memory 同步开发" in content
